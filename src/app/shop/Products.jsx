...
export default function Product() {
  ...
  const denimData = [
    ...
    },
    {
      ...
      image: "",
      name: "Vintage Embroidered Denim Jacket",
      category: "Tops",
      price: 7090.99,
      description:
    ...
    },
    {
      ...
      tags: ["denim", "jacket", "outerwear", "men"],
      sizes: ["XS", "S", "M", "L", "XL"],
    },
    {
      id: 6,
 ...
 const filteredLists = denimData.filter((data) =>
    data.name.toLowerCase().includes(sort.toLowerCase()) || 
    data.category.toLowerCase().includes(sort.toLowerCase())
  );

  const denimCart = filteredLists.map((cart) => (
    <section key={cart.id}>
      <Link href={`/category/${cart.name.split("").join("-")}`}>
        <Image src={cart.image} alt={cart.name} width="400" height="400" />
        <h2>{cart.name}</h2>
        <p>&#x20A6;{cart.price}</p>
      </Link>
    </section>
  ...
  return (
    <>
      <div className="flex items-center border-4 border-gray-600 rounded-full m-auto w-1/2">
        <input
          type="text"
          name="text"
          id="text"
      ...
      <div
        className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-10 px-5"
      >
        {filteredLists == 0 ? <h2 className=" ">No items Found</h2> : denimCart}
      </div>
    </>
  );
}
